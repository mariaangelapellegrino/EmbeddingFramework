...
class Evaluator:
	...
	def evaluate(vectors, distance_metric, results_folder):
		...
		if data.size == 0:
			...
			scores.append({'task_name' : 'Document Similarity', 'gold_standard_file' : 'LP50', 'conf': 'ignoring weight', 'pearson_score': pearson_score, 'spearman_score' : spearman_score, 'harmonic_mean' : harmonic_mean})

			doc_similarity = model.Model.compute_doc_distance(data, distance_metric, True)
			gold_similarity_score, similarity_score = model.Model.get_gold_and_actual_score(stats, doc_similarity)
			pearson_score, spearman_score, harmonic_mean = model.Model.evaluate_document_similarity(gold_similarity_score, similarity_score)	
			
			scores.append({'task_name' : 'Document Similarity', 'gold_standard_file' : 'LP50', 'conf': 'with weight', 'pearson_score': pearson_score, 'spearman_score' : spearman_score, 'harmonic_mean' : harmonic_mean})

		with open(results_folder+'/documentSimilarity_LP50.csv', "wb") as csv_file:
